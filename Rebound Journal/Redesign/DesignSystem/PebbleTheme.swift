...
enum PebbleTheme {
    ...
    /// 시트 맨 위 제목까지의 여백.
    ///
    /// 시트는 위쪽 모서리가 크게 둥글고, 그 자리에 손잡이가 놓인다. 여느 화면과
    /// 같은 여백(12pt)을 주면 제목이 그 곡선 안쪽으로 파고들어 **화면이 위로
    /// 잘려 나간 것처럼 보인다.** 실제로 잘린 건 없는데도 그렇게 읽힌다.
    ///
    /// 곡선과 손잡이가 차지하는 만큼을 비워 둔다.
    static let sheetHeaderTop: CGFloat = 30
